...
for i, n in enumerate(l1):
    flats.Flat.findByFlatID(flats.getAllHouseFlats(house_data), n).wall_residents = [l2[i]]
    flats.Flat.findByFlatID(flats.getAllHouseFlats(house_data), l2[i]).wall_residents = [n]
'''
flats.Resident.findByTgID(flats.getAllHouseResidents(house_data), 0).setStatus(2, 0)
func.save_dict_to_file(conf['BOT']['data'], house_data, key=conf['BOT']['cryptokey'])
